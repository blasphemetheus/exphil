defmodule ExPhil.Native.TritonScan do
  @moduledoc """
  NIF bindings to Triton AOT-compiled linear scan kernel (h = a*h + b).

  The kernel is written in Triton's Python DSL, AOT-compiled to cubin at build
  time, and loaded via CUDA driver API at runtime. Zero Python dependency at
  runtime — the NIF is pure C + CUDA driver.

  ## Building

  ```bash
  cd native/triton_scan
  make          # AOT compile (needs Python + Triton) + build NIF
  make install  # Copy .so + .cubin to priv/native/
  ```

  If Triton is not available, you can build just the NIF from a pre-compiled cubin:
  ```bash
  ...
  ```elixir
  if ExPhil.Native.TritonScan.available?() do
    result = ExPhil.Native.TritonScan.linear_scan(a, b, h0)
  end
  ```
  """
  ...
  @doc false
  def load_nif do
    nif_path = Application.app_dir(:exphil, "priv/native/libtriton_scan_nif")
  ...
  @doc """
  Check if the Triton NIF is loaded and working.
  """
  @spec available?() :: boolean()
  def available? do
    try do
      ping() == ~c"pong from triton_scan_nif"
    rescue
      _ -> false
    catch
      _ -> false
  ...
  Uses Triton AOT-compiled cubin loaded via CUDA driver API.
