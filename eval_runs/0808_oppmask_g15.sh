...
# lever BURIED per today's A/B) + --opp-scramble-frames 12000 — the
...
# (GATE V removed — g13's inversion already verified 2026-08-08.)
# g14 = round 3: per-frame opponent SCRAMBLE (true-dropout shape) after
# g13/g13b showed coherent redress raises dependence. References:
#   g10b: fox 421 c421 / mewtwo 125.8 c1 / rung-0 1.44
#   g13b: fox 389.5 c382 / mewtwo 68.9 c3 / rung-0 2.64
# Success = fox >=300 AND mewtwo chain > 3 AND rung-0 < 2.0 (closer to
# g10b than g13b). Rung-0 is the shape test: masking should REDUCE
# opponent consultation where redress raised it.
echo "=== G15 TRAIN $(date +%H:%M:%S)"
...
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g15_oppmask_full.bin eval_runs/0808_g15_stand_fox \
...
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g15_oppmask_full.bin eval_runs/0808_g15_stand_mewtwo \
  --runs 1 --dummy stand --runner sync \
...
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g15_oppmask_full.bin eval_runs/0808_g15_ys \
...
EXLA_TARGET=host mix run scripts/probe_opponent_dependence.exs \
  --policies "checkpoints/ms_g15_oppmask_full.bin" 2>&1 | tail -5 || true
