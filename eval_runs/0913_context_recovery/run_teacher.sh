...
OUT=eval_runs/0913_context_recovery/teacher
 test ! -e "$OUT"
mkdir -p "$OUT"
...
mix run --no-compile --no-deps-check "${LOAD[@]}" \
  ...
  scripts/scenario_suite.exs --driver teacher --audit-teacher-labels --policy eval_runs/0913_no_dropout_fit/round21/candidate.bin \
  --reaction-delay 2 --temperature 1.0 --character fox --prefix-history committed \
  --manifest eval_runs/0913_interruption_recovery/manifest.json --runs 1 --window 360 --response-opponent neutral --live-af \
