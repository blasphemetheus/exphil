...
OUT=eval_runs/0913_context_recovery/warm_recheck
 test ! -e "$OUT"
mkdir -p "$OUT"
export EXPHIL_SKIP_NIF_COMPILE=1 EXLA_TARGET=cuda EXPHIL_GPU_MEMORY_FRACTION=0.15
LOAD=(-r lib/exphil/constants.ex -r lib/exphil/eval/scenario_opponent.ex -r ../edifice/lib/edifice/recurrent/recurrent.ex
...
mix run --no-compile --no-deps-check "${LOAD[@]}" \
  ...
  --manifest eval_runs/0913_matched_handoffs/manifest.json --runs 2 --window 120 --response-opponent replay --live-af \
