#!/usr/bin/env bash
# g21 (2026-09-10 afternoon) = g20 recipe aimed at SHARPNESS under sampling.
...
# Changes vs g20: ONE rung (--multi-delay "0", the local no-delay rung: no
...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head independent --clean-loss"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g21a ;;
  b) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g21b ;;
...
echo "=== G21 TRAIN $(date +%H:%M:%S) (g19 recipe, causal labels, AR head, clean loss, rungs 0-3, 24ep, snapshot-all)"
EXPHIL_GPU_MEMORY_FRACTION=0.75 mix run scripts/dagger_drill.exs \
  ...
  --action-delay 1 --multi-delay "0" --pipeline-offset 2 --queue-depth 4 --with-delay-id \
  $SS_ARGS --awbc \
  $HEAD_ARGS \
  --lr 2.0e-4 \
  --target-loss 1.0e-9 \
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G21 TRAIN FAILED" >&2; exit 1; }
echo "=== G21 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
