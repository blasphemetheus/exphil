...
bash eval_runs/0915_float_input/smoke12.sh
jq -e '.errored_runs == 0 and .diverged_runs == 0 and ([.runs[] | select(.prefix_audit.valid == true)] | length) == 2' eval_runs/0915_float_input/smoke12/report.json >/dev/null
OUT=eval_runs/0915_float_input/coverage03
R=$OUT/round21
test ! -e "$R" || { echo "$R exists" >&2; exit 1; }
mkdir -p "$R"
K=4; Q=5
P=$OUT/progress.log
...
sha256sum "$0" scripts/{dagger_drill,measure_teacher_fit,check_early_teacher_fit,scenario_suite,score_interruption_recovery}.exs \
  "$FIXTURE" eval_runs/0914_delay4_proof/clips/*.frames $OUT/clips_train/*.frames $OUT/clips_heldout/*.frames > "$R/sources.sha256"
mix run scripts/dagger_drill.exs \
  --expert multishine --fixture "$FIXTURE" \
  --recurrent-state zeros --precision f32 \
  --recorded-frames "$TRAIN_GLOB" \
...
say "  $(grep -E 'Recorded prefix sampling' $R/train.log | sed 's/^\[[0-9:]*\] //' | cut -c1-160)"
say "  $(grep -oE 'epoch (1|9|21)/21: loss=[0-9.e-]+' $R/train.log | tr '\n' ' ' || echo 'TRAIN FAILED')"
...
live() { local name=$1 manifest=$2 history=$3 window=$4 opponent=$5
  say "live $name"
  EXTRA=()
  if [[ $name == cov_* ]]; then EXTRA=(--float-ports 1,2 --no-pipe-shim); fi
  mix run scripts/scenario_suite.exs --driver policy --policy "$R/candidate.bin" "${COMMON[@]}" --dolphin /home/blewf/.local/share/slippi/exi-ai-float-rng-v2b/dolphin-emu-headless "${EXTRA[@]}" \
