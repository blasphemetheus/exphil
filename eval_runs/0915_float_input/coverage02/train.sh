...
set -euo pipefail
echo "HALT: input equality exposed RNG-dependent state drift; validate protocol v2 before training."
exit 1
cd /home/blewf/git/exphil
if pgrep -x beam.smp >/dev/null; then echo "A BEAM is live; wait." >&2; exit 1; fi
OUT=eval_runs/0915_float_input/coverage02
R=$OUT/round21
...
live() { local name=$1 manifest=$2 history=$3 window=$4 opponent=$5
  ...
  mix run scripts/scenario_suite.exs --driver policy --policy "$R/candidate.bin" "${COMMON[@]}" --dolphin /home/blewf/.local/share/slippi/exi-ai-float-v3/dolphin-emu-headless "${EXTRA[@]}" \
