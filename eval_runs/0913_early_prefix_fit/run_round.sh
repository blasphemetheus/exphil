...
OUT=eval_runs/0913_early_prefix_fit/round21
test ! -e "$OUT"
mkdir -p "$OUT"
...
sha256sum scripts/{dagger_drill,measure_teacher_fit,check_early_teacher_fit}.exs \
  lib/exphil/training/{data,labels,recorded_prefix_sampling,epoch_loss}.ex \
  eval_runs/0913_tiny_overfit/initial.bin \
...
mix run --no-compile --no-deps-check "${LOAD[@]}" -r lib/exphil/eval/teacher_fit.ex \
  scripts/measure_teacher_fit.exs --policy "$OUT/candidate.bin" --out "$OUT/fit.json" \
  > "$OUT/fit.log" 2>&1
mix run --no-compile --no-deps-check --no-start -r lib/exphil/eval/early_teacher_gate.ex \
  -r lib/exphil/networks/policy/execution_contract.ex \
