...
mix run scripts/probe_snapshot_health.exs --policies "${N}_ep33.bin,${N}_ep34.bin,${N}_ep35.bin,${N}_ep36.bin,${N}_ep37.bin,${N}_ep38.bin,${N}_ep42.bin,${N}_ep43.bin,${N}_ep48.bin" --delay-id 4 --offset 4 --out $O/guard_health.json 2>&1 | grep -aE "ms_g26a_ep|wrote|error"
...
GATE_REACTION=4 EPOCHS="33 34 36 38 42 48" bash scripts/gate_sweep.sh $N $O/stand_guard 2>&1 | grep -aE "^ep[0-9]+:|ARGMAX"
echo "=== 4. CPU gate k=4 ($(date +%H:%M))"
bash eval_runs/0911_g24_dagger/gate_cpu.sh $N $O/cpu_guard "33 34 38 48" 2>&1 | tail -6
echo "=== DONE $(date +%H:%M)"
