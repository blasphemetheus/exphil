#!/usr/bin/env bash
# G26 (2026-09-12 evening, Bradley: "Go" on the phase-indexed k-ahead expert):
# the g24a POOL (no redresses — isolates the label fix) with physical delay-ids
# (--multi-delay "3,4,5") and INVARIANTS item 14 labels: expert-labeled lists
# (relabeled rollouts, openers, snippets) get their delayed labels from
# MultishineExpert.label_ahead (the loop's own future), never from shifting the
# recorded (student) future. The audit below runs AT the training shifts and
# gates: g25's pool showed 10/11 loop states conflicting at shift 4 under the
# old labels; this pool must show 0. Reads: map (offset 4) vs ep57 and g25a,
# then stand floor + CPU gate at --reaction-delay 4 — the floor (296 -> 111 ->
# 93 across DAgger rounds) is the number this fix is meant to restore.
# g24 (2026-09-11 01:00) = g23a recipe + DAgger round on the FAILING states.
# Bradley (00:5x): the goal is to multishine no matter where Fox is or what the
# opponent does — a methodology test. ms_g23a_ep57 does 296/min c277 vs a
# standing dummy but 54-62/min c2-3 vs the level-1 CPU and 35.7/min c2 vs
# Bradley: state coverage, not delay. Rollouts added: Bradley's 0911 session +
# 8 CPU-dummy games of ep57 (relabeled by the expert, the drill's own loop).
# GATE = --dummy cpu shines/min (stand stays as the technique floor). Arms:
#   a = with snippets (g23a)     b = without (g23b)
# SNIPPET LABEL CONFLICT removed. g22b fit its pool to 5e-5 yet sat at 0.95/0.96
# on the two boundary frames of the fixture -> a 95/5 label mix at those states.
# Cause: snippet_mine.exs built its expert table from fox_multishine_closed.slp
# while the drill uses closed_d1.slp (one frame earlier) — every arm today AND
# g19 trained on 12.5k snippet frames one frame off at the boundaries.
# Arms: a = snippets re-mined with the d1 fixture; b = NO snippets (clean control).
...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g26a; SNIP_ARGS="--snippet-frames $SNIP" ;;
  b) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g26b; SNIP_ARGS="" ;;
...
mix run scripts/audit_ms_pool_labels.exs --rollouts "$ROLL" --openers "$OPEN" ${SNIP_ARGS:+--snippets "$SNIP"} --shifts "3,4,5" 2>&1 | grep -aE "CONFLICT|conflict" || { echo "=== pool label audit FAILED" >&2; exit 3; }
echo "=== G26 TRAIN $(date +%H:%M:%S) (g19 recipe, causal labels, AR head, clean loss, rungs 0-3, 24ep, snapshot-all)"
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G26 TRAIN FAILED" >&2; exit 1; }
echo "=== G26 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
echo "  mix run scripts/probe_ms_coverage_map.exs --policy checkpoints/${NAME}_ep<best>.bin --delay-id 4 --out $OUT/map_ep<best>.json   # FIRST: map before any gate"
echo "  GATE_REACTION=4 bash scripts/gate_sweep.sh checkpoints/$NAME $OUT/sweep_${ARM}_k4"
echo "  bash eval_runs/0911_g24_dagger/gate_cpu.sh checkpoints/$NAME $OUT/cpu_${ARM} "<epochs>""
