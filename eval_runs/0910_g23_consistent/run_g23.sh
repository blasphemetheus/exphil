#!/usr/bin/env bash
# g23 (2026-09-10 17:50) = g22b recipe (buckets 24, 4 rungs, 60 ep) with the
...
OUT=eval_runs/0910_g23_consistent
...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g23a; SNIP_ARGS="--snippet-frames $SNIP" ;;
  b) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g23b; SNIP_ARGS="" ;;
...
echo "=== G23 TRAIN $(date +%H:%M:%S) (g19 recipe, causal labels, AR head, clean loss, rungs 0-3, 24ep, snapshot-all)"
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G23 TRAIN FAILED" >&2; exit 1; }
echo "=== G23 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
