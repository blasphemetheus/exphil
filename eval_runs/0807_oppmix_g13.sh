...
# lever BURIED per today's A/B) + --opp-randomize-frames 12000 — the
...
echo "=== GATE 0: g10b stand-MEWTWO baseline (the missing reference) $(date +%H:%M:%S)"
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g10b_human.bin eval_runs/0807_g10b_stand_mewtwo \
...
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g13_oppmix.bin eval_runs/0807_g13_stand_mewtwo \
  --runs 1 --dummy stand --runner sync \
  -- --frame-delay 3 --dummy-character mewtwo --headless --emulation-speed 0 --blocking-input --slippi-port 51442

echo "=== GATE 3: YS 3 runs (bucket only, n>=8 for claims)"
EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh \
  checkpoints/ms_g13_oppmix.bin eval_runs/0807_g13_ys \
...
EXLA_TARGET=host mix run scripts/probe_opponent_dependence.exs \
  --policies "checkpoints/ms_g13_oppmix.bin" 2>&1 | tail -5 || true
