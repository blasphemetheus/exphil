...
NAME="${1:?initial or candidate}"
case "$NAME" in initial|candidate) ;; *) exit 1;; esac
test ! -e "$OUT/eval_$NAME.json"
test ! -e "$OUT/eval_$NAME.log"
...
mix run --no-compile --no-deps-check \
  ...
  -r lib/exphil_bridge/melee_port.ex -r lib/exphil/eval/scenario_input_timing.ex \
  -r lib/exphil/eval/scenario_history.ex -r lib/exphil/eval/recovery_label_audit.ex \
  scripts/scenario_suite.exs --driver policy --policy "$OUT/$NAME.bin" \
  --reaction-delay 2 --temperature 1.0 --character fox --prefix-history cold \
  --manifest eval_runs/0913_tiny_overfit/manifest.json --runs 2 \
