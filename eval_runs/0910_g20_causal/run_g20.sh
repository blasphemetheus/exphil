...
#   * --max-epochs 24 (g19's peak was ep3-8; sweeping 3 rungs x 60 is
...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head autoregressive --clean-loss"; SS_ARGS=""; NAME=ms_g20a ;;
  b) HEAD_ARGS="--head independent --clean-loss"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g20b ;;
...
echo "=== G20 TRAIN $(date +%H:%M:%S) (g19 recipe, causal labels, AR head, clean loss, rungs 0-3, 24ep, snapshot-all)"
EXPHIL_GPU_MEMORY_FRACTION=0.75 mix run scripts/dagger_drill.exs \
  ...
  --max-epochs 24 --prev-action-dropout 0.6 --transition-weight 2.0 \
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G20 TRAIN FAILED" >&2; exit 1; }
echo "=== G20 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
