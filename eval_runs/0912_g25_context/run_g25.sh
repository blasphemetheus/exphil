#!/usr/bin/env bash
# G25 (2026-09-12 evening, Bradley: "build it"): g24a recipe + units fixed + the
# coverage-map levers. Delay-ids PHYSICAL (--multi-delay "3,4,5" == g24 "1,2,3"
# + the retired pipeline offset 2; k=4 = local async rung, --reaction-delay 4).
# --mirror-frames 12000 (map: mirror not learned) and --opp-context-frames 16000
# (map: opp shield/grab/shine + behind/close kill the JC out of shine). Reads,
# in order: (1) coverage map on the best snapshot vs eval_runs/0912_coverage_map/
# ep57_id0.json (red cells rise, baseline holds); (2) stand floor k=4; (3) CPU
# gate k=4. Both live numbers must hold (g24a lesson). Header below = g24 history.
...
OUT=eval_runs/0912_g25_context
...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g25a; SNIP_ARGS="--snippet-frames $SNIP" ;;
  b) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g25b; SNIP_ARGS="" ;;
...
echo "=== G25 TRAIN $(date +%H:%M:%S) (g19 recipe, causal labels, AR head, clean loss, rungs 0-3, 24ep, snapshot-all)"
EXPHIL_GPU_MEMORY_FRACTION=0.75 mix run scripts/dagger_drill.exs \
  ...
  --mirror-frames 12000 --opp-context-frames 16000 \
  $SS_ARGS --awbc \
  $HEAD_ARGS \
  --lr 2.0e-4 \
  --target-loss 1.0e-9 \
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G25 TRAIN FAILED" >&2; exit 1; }
echo "=== G25 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
