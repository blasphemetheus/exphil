...
ARM="${ARM:-a}"
case "$ARM" in
  a) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g24a; SNIP_ARGS="--snippet-frames $SNIP" ;;
  b) HEAD_ARGS="--head independent --clean-loss --action-frame-buckets 24"; SS_ARGS="--scheduled-sampling 0.5 --ss-ramp 10"; NAME=ms_g24b; SNIP_ARGS="" ;;
...
[ -f "checkpoints/$NAME.bin" ] || { echo "=== G24 TRAIN FAILED" >&2; exit 1; }
echo "=== G24 TRAIN DONE $(date +%H:%M:%S). Gate-sweep next (foreground, chunked):"
