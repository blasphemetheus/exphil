...
for ep in $EPOCHS; do
  ...
  EXLA_TARGET=host EXPHIL_GPU_MEMORY_FRACTION=0.25 bash scripts/eval_live_protocol.sh "$snap" "$OUT/ep$ep" --runs 2 --seconds 90 --dummy cpu --runner async --temperature 1.0 -- --reaction-delay 4 --headless --emulation-speed 0 --blocking-input --slippi-port 51442 2>&1 | grep -aE "^\[[0-9:]+\] r[12] " | sed "s/^/ep$ep /" | tee -a "$OUT/cpu_table.txt"
