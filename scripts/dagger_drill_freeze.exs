...
# --multi-delay: see dagger_drill.exs (queue-as-input mixed-delay pool).
delays =
...
dataset =
  if streaming do
    ...
    |> then(fn ds ->
      # Queue-as-input: see dagger_drill.exs — config carries the layout.
...
# ============================================================================
# FREEZE-TRUNK variant (2026-07-31, HANDOFF_2026-07-29b mains item 5).
# Warm-start every parameter from a donor policy, then train HEADS ONLY:
# the optimizer wrapper zeroes trunk gradients before the update (AdamW's
# decoupled decay still touches the trunk, so export splices the donor
# trunk back verbatim — within-lineage splice, valid unlike the deleted
# cross-lineage hybrids). If the delay preference reproduces, adaptation
# is head-implementable: shifted thresholds over unchanged features.
# Donor via FREEZE_DONOR env (arch must match: gru w16 h256 + prev-action).
# ============================================================================
freeze_donor = System.get_env("FREEZE_DONOR") || raise("FREEZE_DONOR required")
{:ok, %{params: donor_params}} = ExPhil.Training.load_policy(freeze_donor)

donor_data =
  case donor_params do
    %Axon.ModelState{data: d} -> d
    %{} = m -> m
  end

head_key? = fn key ->
  Enum.any?(~w(buttons_ main_x_ main_y_ c_x_ c_y_ shoulder_), &String.starts_with?(key, &1))
end

zero_tree = fn zero_tree, node ->
  case node do
    %Nx.Tensor{} = t -> Nx.multiply(t, 0)
    %{} = m -> Map.new(m, fn {k, v} -> {k, zero_tree.(zero_tree, v)} end)
    other -> other
  end
end

Output.puts(
  "[freeze-trunk] donor #{freeze_donor}: #{map_size(donor_data)} layers, " <>
    "heads trainable: #{donor_data |> Map.keys() |> Enum.count(head_key?)}"
)

orig_optimizer = trainer.optimizer

masked_optimizer = fn grads_data, opt_state, params_data ->
  masked =
    Map.new(grads_data, fn {k, v} ->
      if head_key?.(k), do: {k, v}, else: {k, zero_tree.(zero_tree, v)}
    end)

  orig_optimizer.(masked, opt_state, params_data)
end

# Merge donor weights over the fresh init: donor bins lack the recurrent
# hidden-state init entries the trainer's ModelState carries — those keep
# their fresh values (they are state seeds, not learned weights).
merged_data = Map.merge(trainer.policy_params.data, donor_data)

trainer = %{
  trainer
  | policy_params: %{trainer.policy_params | data: merged_data},
...
# [freeze-trunk] splice the donor trunk back verbatim (undoes AdamW decay
# on the frozen layers; heads keep their fine-tuned values).
best_trainer =
  (fn tr ->
     spliced =
       Map.new(tr.policy_params.data, fn {k, v} ->
         if head_key?.(k), do: {k, v}, else: {k, Map.get(donor_data, k, v)}
       end)

     %{tr | policy_params: %{tr.policy_params | data: spliced}}
   end).(best_trainer)
