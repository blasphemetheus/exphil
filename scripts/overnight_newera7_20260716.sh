#!/usr/bin/env bash
# NEW-ERA DAgger rounds 11-12 (2026-07-16): first training with the two
# new fixtures (mewtwo_oos_chains: 138 short shield-exits, targets the
# shield-p95 gate failing in every r9/r10 game; mewtwo_ground_neutral:
# dash-dance/wavedash spacing, replaces synthetic movement defaults).
# Pool = 36 games (25 + r9/r10 probe replays). Same objectives, gates,
# and 3+3 plain/debounce A/B as newera6. Scores comparable to r1-r10.
set -uo pipefail

if [ -z "${NEWERA7_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NEWERA7_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="newera7" \
...
LOG=logs/overnight_newera7_20260715.log
