...
set -uo pipefail
if [ -z "${NANDIAG_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NANDIAG_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="nandiag" \
    --why="NaN forensics run in progress" "$0" "$@"
fi
cd "$(dirname "$0")/.."
LOG=logs/diagnose_nan_20260713.log
if pgrep -x beam.smp >/dev/null; then
  echo "[nandiag] BEAM ALREADY LIVE — refusing to start (NO-MIX rule)" | tee -a "$LOG"
  exit 1
fi
POOL=(
...
echo "[nandiag] === forensics run: 42-pool, LR 2e-4, pure bf16, --nan-forensics ===" | tee -a "$LOG"
mix run scripts/dagger_drill.exs \
  --expert mewtwo_combo \
  --max-epochs 150 \
  --nan-forensics \
  --rollouts "$ROLLOUTS" \
  --out checkpoints/scratch_nandiag_policy.bin >>"$LOG" 2>&1
echo "[nandiag] exited with code $? (2 = forensics NaN dump captured)" | tee -a "$LOG"
