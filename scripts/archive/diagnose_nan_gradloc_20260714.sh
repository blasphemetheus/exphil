...
set -uo pipefail
if [ -z "${NANDIAGGL_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NANDIAGGL_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="nandiag_gradloc" \
    --why="grad-localizer NaN run in progress" "$0" "$@"
fi
cd "$(dirname "$0")/.."
LOG=logs/diagnose_nan_gradloc_20260714.log
if pgrep -x beam.smp >/dev/null; then
  echo "[nandiag_gradloc] BEAM ALREADY LIVE — refusing to start (NO-MIX rule)" | tee -a "$LOG"
  exit 1
fi
POOL=(
...
echo "[nandiag_gradloc] === grad localizer: w16 recipe + per-step grad checks from step 250k ===" | tee -a "$LOG"
mix run scripts/dagger_drill.exs \
  ...
  --rollouts "$ROLLOUTS" \
  --out checkpoints/scratch_nandiag_gradloc_policy.bin >>"$LOG" 2>&1
echo "[nandiag_gradloc] exited with code $? (2 = forensics NaN dump captured)" | tee -a "$LOG"
