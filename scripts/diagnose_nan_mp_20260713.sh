...
set -uo pipefail
if [ -z "${NANDIAGMP_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NANDIAGMP_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="nandiag_mp" \
    --why="mixed-precision NaN control in progress" "$0" "$@"
fi
cd "$(dirname "$0")/.."
LOG=logs/diagnose_nan_mp_20260713.log
if pgrep -x beam.smp >/dev/null; then
  echo "[nandiag_mp] BEAM ALREADY LIVE — refusing to start (NO-MIX rule)" | tee -a "$LOG"
  exit 1
fi
POOL=(
...
echo "[nandiag_mp] === MP control: 42-pool, LR 2e-4, f32 master + bf16 compute, forensics on ===" | tee -a "$LOG"
mix run scripts/dagger_drill.exs \
  ...
  --mixed-precision \
  --rollouts "$ROLLOUTS" \
  --out checkpoints/scratch_nandiag_mp_policy.bin >>"$LOG" 2>&1
echo "[nandiag_mp] exited with code $? (2 = forensics NaN dump captured)" | tee -a "$LOG"
