...
set -uo pipefail
if [ -z "${NANDIAGF32_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NANDIAGF32_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="nandiag_f32loss" \
    --why="f32-loss confirmation run in progress" "$0" "$@"
fi
cd "$(dirname "$0")/.."
LOG=logs/diagnose_nan_f32loss_20260714.log
if pgrep -x beam.smp >/dev/null; then
  echo "[nandiag_f32loss] BEAM ALREADY LIVE — refusing to start (NO-MIX rule)" | tee -a "$LOG"
  exit 1
fi
POOL=(
...
echo "[nandiag_f32loss] === f32-loss confirmation: same recipe, loss cast to f32 — expect survival past 500k ===" | tee -a "$LOG"
mix run scripts/dagger_drill.exs \
  --expert mewtwo_combo \
  --max-epochs 150 \
  --nan-forensics \
  --window 16 \
  --debug-grads-after 250000 \
  --rollouts "$ROLLOUTS" \
  --out checkpoints/scratch_nandiag_f32loss_policy.bin >>"$LOG" 2>&1
echo "[nandiag_f32loss] exited with code $? (2 = forensics NaN dump captured)" | tee -a "$LOG"
