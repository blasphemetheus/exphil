...
if [ -z "${NEWERA_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NEWERA_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="newera" \
...
LOG=logs/overnight_newera_20260714.log
