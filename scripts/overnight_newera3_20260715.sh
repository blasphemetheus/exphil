...
if [ -z "${NEWERA3_INHIBITED:-}" ] && command -v systemd-inhibit >/dev/null 2>&1; then
  export NEWERA3_INHIBITED=1
  exec systemd-inhibit --what=sleep --who="newera3" \
...
LOG=logs/overnight_newera3_20260715.log
...
# Recover round 4 evaluation first: its training survived the mid-loop
# compile crash (see CLAUDE.md loop-edit rule) but its probes died.
for p in 1 2 3; do
  PROBE_REPLAY=""
  if probe "checkpoints/mewtwo_combo_newera_r4_policy.bin" "r4_p${p}" && [ -n "$PROBE_REPLAY" ]; then
    POOL+=("$PROBE_REPLAY")
  fi
done

for R in $(seq 5 $((4 + ROUNDS))); do
