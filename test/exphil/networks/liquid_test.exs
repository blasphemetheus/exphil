defmodule ExPhil.Networks.LiquidTest do
  @moduledoc """
  Tests for Liquid Neural Networks (LNN).
  ...
  describe "build/1" do
    test "builds model with correct output shape (Euler)" do
      model =
        Liquid.build(
    ...
    test "builds model with correct output shape (RK4)" do
      model =
        Liquid.build(
          embed_size: @embed_size,
          hidden_size: @hidden_size,
    ...
    test "handles multiple integration steps" do
      for steps <- [1, 2, 4] do
        model =
          Liquid.build(
            embed_size: @embed_size,
        ...
        assert Nx.shape(output) == {@batch_size, @hidden_size},
               "Failed for integration_steps=#{steps}"
      end
    end
  ...
  describe "build_with_ffn/1" do
    test "builds model with FFN layers" do
      model =
  ...
  describe "output_size/1" do
    test "returns hidden_size" do
      ...
      assert Liquid.output_size(hidden_size: 256) == 256
      # default
  ...
  describe "melee_defaults/0" do
    test "returns valid default configuration" do
      defaults = Liquid.melee_defaults()
  ...
  describe "numerical stability" do
    ...
    test "RK4 produces finite outputs" do
      model =
        Liquid.build(
          embed_size: @embed_size,
          hidden_size: @hidden_size,
  ...
  describe "integration with Policy" do
    test "works as liquid temporal backbone in Policy" do
