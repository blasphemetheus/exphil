defmodule ExPhil.Networks.GatedSSMTest do
  ...
  describe "build/1" do
    test "builds GatedSSM model with correct output shape" do
      model =
        GatedSSM.build(
    ...
    test "builds stacked GatedSSM layers" do
      model =
        GatedSSM.build(
          embed_size: @embed_size,
          hidden_size: @hidden_size,
    ...
    test "respects expand_factor option" do
      # expand_factor controls the inner dimension of the GatedSSM block
      model =
        GatedSSM.build(
          embed_size: @embed_size,
          hidden_size: @hidden_size,
  ...
  describe "build_mamba_block/2" do
    test "builds a single GatedSSM block with correct shape" do
  ...
  describe "build_causal_conv1d/4" do
    test "maintains sequence length after causal convolution" do
      ...
      conv = GatedSSM.build_causal_conv1d(input, @hidden_size, 4, "test_conv")
    ...
    test "works with different kernel sizes" do
      for kernel_size <- [2, 4, 8] do
        ...
        conv = GatedSSM.build_causal_conv1d(input, @hidden_size, kernel_size, "conv_k#{kernel_size}")
  ...
  describe "build_selective_ssm/2" do
    test "produces correct output shape" do
      ...
      ssm =
        GatedSSM.build_selective_ssm(input,
          hidden_size: @hidden_size,
          state_size: @state_size,
          name: "test_ssm"
        )
  ...
  describe "output_size/1" do
    test "returns hidden_size" do
      ...
      assert GatedSSM.output_size(hidden_size: 256) == 256
      # default
  ...
  describe "melee_defaults/0" do
    test "returns valid default configuration" do
      defaults = GatedSSM.melee_defaults()
    ...
    test "builds a valid model with defaults" do
      ...
      model = GatedSSM.build([embed_size: 1991] ++ defaults)
  ...
  describe "step/4" do
    ...
    test "updates cache state across multiple steps" do
      ...
      {out1, cache} = GatedSSM.step(x1, params, cache)

      x2 = Nx.broadcast(0.7, {1, @hidden_size})
      {out2, cache} = GatedSSM.step(x2, params, cache)

      x3 = Nx.broadcast(0.3, {1, @hidden_size})
      {out3, cache} = GatedSSM.step(x3, params, cache)
    ...
    test "accepts [batch, 1, hidden] input shape" do
      ...
      {output, _cache} = GatedSSM.step(x, params, cache)
    ...
    test "incremental inference produces valid outputs for each step" do
      ...
      incremental_outputs =
        Enum.reduce(0..(seq_len - 1), {[], cache}, fn i, {outputs, cache} ->
          ...
          {out, new_cache} = GatedSSM.step(x, params, cache)
