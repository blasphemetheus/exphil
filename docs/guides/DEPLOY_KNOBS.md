...
| **Train-delay -> live flag (INVARIANTS item 1, 2026-09-09)** | trained at reaction delay k (the training `--frame-delay`/`--action-delay` since 09-09; legacy checkpoints: their delay minus 1) -> deploy `--frame-delay k+1`. Delay-id is derived by the Agent from the checkpoint's `label_convention` (causal: id N-1, legacy: id N); `--delay-id-override` still wins. | `ExPhil.Data.LabelConvention` (the law + the deploy cards as tests). The live flag was NOT renumbered — every row in this table keeps its numbers. v16e (legacy `--frame-delay 1`) and a v3 causal checkpoint at the default both deploy at `--frame-delay 1`. |
