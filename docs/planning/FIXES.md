...
| File | Line | TODO |
|------|------|------|
| ~~`mamba_ssd.ex`~~ | ~~286~~ | ~~"Optimize by using precomputed chunk_a_products"~~ (done: dead code removed, cumprod replaced with Nx.cumulative_product) |
...
### 4.1b MambaSSD Fused CUDA NIF for Inference [P3]

The XLA-based SSD matmul path (~7 vectorized ops) is fast but a fused CUDA NIF could eliminate kernel launch overhead entirely. Target: sub-1ms inference for real-time play.
...
### 8.3 Edifice Unused Function Warning [P3]

```
warning: function rla_backward_dispatch/10 is unused
└─ lib/edifice/cuda/fused_scan.ex:1589:8: Edifice.CUDA.FusedScan (module)
```

**Fix:** Remove or `@doc false` + prefix with `_`, or wire up the backward pass if it's intended for use.

### 8.4 Flash Attention NIF Warnings [P3]
