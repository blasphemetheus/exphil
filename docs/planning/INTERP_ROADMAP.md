...
|---|---|---|---|
| 2026-07-20 | P6→training | **Probe-as-regularizer WRITTEN (#36, uncompiled)**: the steering verdict's training-time analog. Online class-mean contrast (shield family 178..182) refit every K epochs from current activations (fresh rounds can't reuse r14's vector — wrong basis); loss += W·mean((trunk·v)²), direction as a jit argument, zero-vector inert start, INLP-style self-limiting. `--probe-reg W --probe-reg-every K` on the drill. **STALE-CLOSED 2026-08-02: this landed and ran** — r15 (07-21, mamba_2 full recipe, probe-reg 0.1/every-5): card 8.3/10, conversions 4→7, deficit report says probe-reg did NOT over-suppress (opp_kd 0.889 ≈ screen); refit path validated r16 + r24c (`refit OK, shield_rows 165`). This log line was simply never updated. | DONE via r15 (see HANDOFF_2026-07-22) |
...
| 2026-07-15 | P3 | **r8 (sighted expert + turnaround fixture) FAILS the pre-registered bar.** interp_p3_case3 on r1/r7/r8, 2,816 rows (1,615 behind): opp_position behind-share r1 7.87% → r7 7.46% → r8 **7.92%** (bar: materially >8%; old-era r1 already sat at 7.9%, so behind-share is FLAT across the program). Front-share does climb (5.6→6.9→7.2) and trunk opp_behind decodability climbs steadily (0.706→0.775→**0.807**) — the trunk knows ever more about opponent side; heads still don't consult it. Probes: 0.0% conversions ×2 (p1 lost to a Dolphin-login mishap), eye test: policy dominated by a jump→double-jump loop (see same-day entry below). Interpretation: fixture+sighted-key moved the REPRESENTATION, not the POLICY; behind-frame behavior is masked by the jump-loop pathology — can't measure turnarounds in a policy that's always airborne. Data-level fix (more behind-exemplars) shows diminishing returns; next lever is teacher-default + jump-loop repair, then remeasure. | bar failed; #21 blocked on jump-loop fix |
| 2026-07-15 | P3 | **NEW PATHOLOGY #4 (user eye test, r8 probes): jump→double-jump metronome loop.** Policy jumps then immediately double-jumps near-constantly; short hops absent. Suspect chain (pre-registered, 4 hypotheses): (A) label prior — expert relabel makes jump the modal action (fair expert: "grounded off-script → restart with a jump", "out of range → jump-restart metronome fairs"; fine key's jumps_left dimension keys the DJ-fair variant); (B) conditional teacher × closed-loop drift concentrating time in off-script states; (C) decode artifact — press/release hysteresis (0.45/0.30) oscillation manufacturing re-presses; (D) prev-action self-conditioning (shield-lock's button cousin; dropout 0.4 may not cover buttons). SH timing separately suspect: prev-action dropout destroys press-age signal needed for ≤5f release. Experiment ladder queued: label stats by state bucket → probe-vs-pool state distribution → P(jump) trace vs hysteresis emissions → prev-action ablation → buttons-head attribution at DJ frames. **Experiment A result (scripts/jump_stats.exs, r8 probes vs fixtures): jump rate 5.3 vs 1.0-1.5 per 100 grounded frames; DJ in 89-94%% of air stints vs 13-21%%; liftoff→DJ p50 = 1 FRAME vs 12-16 in data.** The 1f timing has no exemplars — manufactured, not imitated: label prior (A) explains amplification but not timing; mechanism is C (hysteresis re-press at liftoff), D (prev-action self-conditioning), or coarse height_bucket keying DJ-fair exemplars at liftoff (B-variant). Bot X/Y press p50=4f (SH-capable) — SH absence is instant-DJ consumption, not press length. **C/D discriminator (interp_shield_jump_offline on r8 × probe replays): D REFUTED** — prev-action ablation RAISES jump prob (grounded 0.305→0.390, +28%%; prev-action mildly suppresses re-press; dropout exonerated). 21.7%% of ALL frames sit above the press threshold. Verdict: spam lives in the GAME-STATE MAPPING — the teacher's jump-default relabeling ("grounded off-script → jump-restart"; jumps_left keying DJ-fair variants) is burned into the weights. Fix = teacher repair (#17) + retrain; runtime re-press debounce (≥10f) is a candidate band-aid for the 1f DJ, A/B-able without retraining. Shield fix confirmed healthy on r8 (dead zone 0.47%%, longest holdable run 179f). | mechanism named: teacher default; fix via #17 |
| 2026-07-16 | P3 | **OVERNIGHT r9+r10 (repaired teacher + DAgger iteration): case #3 bar CLEARED, jump pathology DEAD, DAgger works.** r9 (25-game pool, turn_toward/approach defaults + af<8 scrub): 6 probes, 4 conversions (best 13.3%%). Gate passed → r10 trained on r9's probe games: **10 conversions (best 20.0%%, three games ended EARLY = stocks closed out)**, report cards up to 7/8. Case #3 saliency behind-share: r8 7.92 → r9 **8.86** → r10 **9.67%%** (pre-registered bar >8 — first clears, and TRENDING); trunk decodability 0.807→0.848→0.875. Jump gates: zero failures across 11 probe games (rate, DJ%%; two near-misses on liftoff p50=6f) — pathology #4 closed by the teacher fix alone. A/B: plain ≈ debounce on conversions (band-aid obsoleted; debounce games took the two 7/8 gate scores — keep as optional polish). **Residual top pathology: LONG SHIELDS** — shield-run p95 215-235f vs 180 gate fails in every game + occasional occupancy fails; next fixture/teacher target (OOS-option chains, task #22 adjacent). | case #3 bar cleared; pathology #4 closed; shield discipline next |
